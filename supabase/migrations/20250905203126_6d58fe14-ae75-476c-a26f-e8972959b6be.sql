...
-- 1) Profiles RLS tightening (drop permissive policy if exists, add strict one)
...
-- 2) Security audit logging table and function (no SELECT triggers; Postgres doesn't support them)
...
-- Enable RLS and deny any direct access by clients
...
-- Create audit function (SECURITY DEFINER so it can insert regardless of RLS)
...
-- Drop any previous audit trigger variants if they exist
...
-- Add write-operation audit triggers
...
-- 3) Consolidate notification_settings RLS into a single strict policy
